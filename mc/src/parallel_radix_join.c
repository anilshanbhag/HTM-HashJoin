...
#warning Only 2-pass partitioning is implemented, set NUM_PASSES to 2!
    ...
    while(1) {
        ...
        DEBUGMSG((my_tid==0), "Got skew task = R: %d\n",
        ...
        numperthr = (*args->skewtask)->relR.num_tuples / args->nthreads;
    ...
    while((task = task_queue_get_atomic(join_queue))){
        /* do the actual join. join method differs for different algorithms,
           ...
           prefetching  */
        results += args->join_function(&task->relR, &task->tmpR);
             ...
             struct timeval * start, struct timeval * end)
{
    ...
    fflush(stdout);
    fprintf(stdout, "%.4lf ", cyclestuple);
    fflush(stdout);
    fprintf(stdout, "\n");
    fflush(stdout);
 ...
 */
int64_t
join_init_run(relation_t * relR, JoinFunction jf, int nthreads)
{
    int i, rv;
...
{
    return join_init_run(relR, bucket_chaining_join, nthreads);
}
