...
int main(int argc, char* argv[]) {
  ...
  uint32_t** conflicts = new uint32_t*[NUM_PARTITIONS];
  ...
  parallel_for(blocked_range<size_t>(0, sizeInTuples, inputPartitionSize),
               [output, tableMask, transactionSize, inputPartitionSize, probeLength,
                ...
                conflictRangeCounts, outputPartitionSize](const auto range) {
                 ...
                 uint32_t randomCounter = 0;
                 uint32_t localConflictRangeCount = 0;
                 uint32_t localPartitionId = range.begin() / inputPartitionSize;
                 uint32_t conflictPartitionStart = outputPartitionSize * localPartitionId;
                 uint32_t* localConflicts = new uint32_t[outputPartitionSize];
                 for(size_t j = range.begin(); j < range.end(); j += transactionSize) {
                   auto status = _xbegin();
                   if(status == _XBEGIN_STARTED) {
                     for(size_t i = j; i < j + transactionSize; i++) {
                       uint32_t inp = input[i];
                       uint32_t curSlot = murmur(inp) & tableMask;
                       ...
                       if (probeBudget == 0) {
                         localConflicts[localConflictCount] = inp;
                 ...
                 conflicts[localPartitionId] = localConflicts;
  ...
  auto conflictSum = parallel_deterministic_reduce(
      blocked_range<size_t>(0, NUM_PARTITIONS, 1), 0ul,
      [&conflicts, &conflictCounts](auto range, auto init) {
        for(size_t i = range.begin(); i < range.end(); i++) {
          for(int j = 0; j < conflictCounts[i]; j++) {
            init += conflicts[i][j];
          }
        }
        return init;
